...
| <a name="input_rbac"></a> [rbac](#input\_rbac) | Provides the ability to associate one of more groups with a sso role in the account | <pre>map(string, object({<br>    users = list(string)<br>    # A list of users to associate with the developer role<br>    groups = list(string)<br>    # A list of groups to associate with the developer role <br>  }))</pre> | `{}` | no |
